...
def step_get_x(action,env):
    _,_,_,state=env.step(action)
    return state['x_position']

def compute_gradient(action,env):
    grad_fn = jax.jacobian(step_get_x)
    return grad_fn(action,env)
...
#   action = torch.zeros(env.action_space.shape).to(device)
  ...
  env.step(action)
  print(compute_gradient(action,env))
